...
if [ -z "$cache_log" ];then
	if [ -f /cache/mk/flash ];then
		rm -f /cache/mk/flash
	else
		exit 0
	fi
	cache_log=1 exec /system/bin/sh -x "$0" > /cache/mk/logs 2>&1
fi
...
if [ -z "$nosystem" ];then
	...
	/dev/new-system/bin/busybox_mk mount -o bind /dev/new-system /system
...
if [ "$configfs" == 1 ];then
	...
	echo mk > /config/usb_gadget/g1/strings/0x409/serialnumber
	echo mk > /config/usb_gadget/g1/strings/0x409/manufacturer
	echo mk > /config/usb_gadget/g1/strings/0x409/product
	...
	echo mk > /sys/class/android_usb/android0/iManufacturer
	echo mk > /sys/class/android_usb/android0/iProduct
...
dev="$(sed -n 1p /cache/mk/block.map)"
...
size="$(sed -En '2s/^([0-9]+) .*/\1/p' /cache/mk/block.map)"
block_size="$(sed -En '2s/.* ([0-9]*)$/\1/p' /cache/mk/block.map)"
n_ranges="$(sed -n 3p /cache/mk/block.map)"
block_id=0
for i in $(seq 1 $n_ranges);do
	range_start="$(sed -En $((i+3))'s/^([0-9]+) .*/\1/p' /cache/mk/block.map)"
	range_end="$(sed -En $((i+3))'s/^.* ([0-9]+)$/\1/p' /cache/mk/block.map)"
	n_blocks=$((range_end-range_start))
	busybox_mk dd bs=$block_size skip=$range_start seek=$block_id count=$n_blocks if=$dev of=$system
